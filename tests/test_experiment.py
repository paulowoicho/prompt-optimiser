...
class TestExperiment:
    def test_automatic_split_is_reproducible_and_keeps_duplicate_inputs_together(self) -> None:
        ...
        assert all(row in train for row in duplicates) or all(
            row in validation for row in duplicates
        )
    ...
    def test_bad_data_fails_before_the_backend_runs(
    ...
    def test_backend_receives_train_and_validation_but_never_test(self, tmp_path: Path) -> None:
        backend = EchoBackend()
        result = optimize(
            problem="Return yes",
            model="fake",
    ...
    def test_nonfinite_metric_is_rejected(self, tmp_path: Path, value: float) -> None:
    ...
    def test_failure_marks_status_failed_and_closes_trackers(self, tmp_path: Path) -> None:
        closed = []
        uploaded = []
    ...
    def test_config_keeps_nested_backend_settings_and_redacts_secrets(
        self, tmp_path: Path
    ) -> None:
        backend = EchoBackend(
            optimizer_kwargs={
    ...
    def test_per_row_predictions_are_saved_for_every_evaluated_split(self, tmp_path: Path) -> None:
        result = optimize(
            problem="Return yes",
            model="fake",
    ...
    def test_training_failure_survives_a_failing_diagnostics_upload(self, tmp_path: Path) -> None:
        closed = []
        ...
        with (
            pytest.warns(RuntimeWarning, match="tracker offline"),
