"""Judge calibration: disjoint splits, vote aggregation and faithful predictor reloads."""
...
from examples.preference_judge import data
from examples.preference_judge import stage2_responses as stage2
...
def _synthetic_pairs() -> list[data.Pair]:
...
class TestPreferenceJudge:
    def test_votes_merge_across_orderings_and_majority_wins_or_ties(self) -> None:
        pd = pytest.importorskip("pandas")

        def conv(question: str, answer: str) -> list[dict[str, str]]:
            return [
        ...
        rows = [
            ...
            dict(
                ...
                winner="model_b",
                turn=1,
                conversation_a=conv("q", "alpaca says"),
                conversation_b=conv("q", "gpt says"),
            ),
            dict(
                question_id=81,
                model_a="gpt-4",
                ...
                winner="model_a",
                turn=1,
                conversation_a=conv("q", "gpt says"),
                conversation_b=conv("q", "alpaca says"),
            ),
            dict(
                question_id=81,
                model_a="alpaca",
            ...
            dict(
                ...
                winner="model_a",
                turn=1,
                conversation_a=conv("q2", "c"),
                conversation_b=conv("q2", "v"),
            ),
            dict(
                question_id=82,
                model_a="claude",
            ...
            dict(
                ...
                winner="model_a",
                turn=2,
                conversation_a=conv("q3", "c"),
                conversation_b=conv("q3", "v"),
            ),
        ]
        pairs = data.pairs_from_frame(pd.DataFrame(rows))
        facts = [
            (p.question_id, p.model_a, p.model_b, p.label, p.votes, p.unanimous) for p in pairs
        ]
        assert facts == [
    ...
    def test_question_splits_are_disjoint_stratified_and_reproducible(self) -> None:
    ...
    def test_judge_examples_contain_both_orderings_with_swapped_labels_and_cap_pairs(self) -> None:
        pairs = _synthetic_pairs()
    ...
    def test_preference_metric_swaps_scores_and_counts_invalid_output(self) -> None:
        ...
        assert seen[0] == {
            "question": "q1",
            "response_a": "a better answer",
            "response_b": "plain",
        }
        assert seen[1] == {
            "question": "q1",
            "response_a": "plain",
            "response_b": "a better answer",
        }
    ...
    def test_load_judge_reloads_textgrad_prompt_and_dspy_program(
        ...
        assert judge('{"question": "q"}') == "TIE" and calls == [
            ("BE A JUDGE", '{"question": "q"}')
        ]
        ...
        assert any(
            message["content"] == "TRAINING DEMO" or "TRAINING DEMO" in message["content"]
            for message in lm.history[-1]["messages"]
        )

    def test_stage2_metric_runs_through_optimize(self, tmp_path: Path) -> None:
        ...
        class TwoPrompts:
            def fit(self, *, seed_prompt: str, **_: Any) -> FitResult:
                return FitResult(
        ...
        assert (
            json.loads((result.output_dir / "report.json").read_text())["preference_vs_seed"][
                "final"
            ]["test"]
            == 1.0
        )

    def test_stage2_reuses_stage1_splits_and_rejects_overlap(self, tmp_path: Path) -> None:
        pairs = _synthetic_pairs()
    ...
    def test_seed_responder_matches_native_adapter_rendering(
        ...
        class KeepSeed:
            def compile(self, student: Any, trainset: Any, valset: Any) -> Any:
                return student
        ...
        trained = DSPy(optimizer=KeepSeed).fit(
            ...
            model=target,
            seed_prompt=seed,
            train=[Example("train", "answer")],
            validation=[Example("val", "answer")],
    ...
    def test_stage1_retained_seed_report_exists_before_artifact_upload(
        self, tmp_path: Path
    ) -> None:
        from examples.preference_judge import stage1_judge as stage1

        class KeepSeed:
            def fit(self, *, seed_prompt: str, **kwargs: Any) -> FitResult:
        ...
        class Upload:
            def log(self, event: Event) -> None:
                if event.kind == "artifact":
                    uploaded.append(
                        json.loads((Path(event.data["path"]) / "report.json").read_text())
                    )
