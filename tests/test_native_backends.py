...
def _splits() -> dict[str, list[Example]]:
    return {
...
def _engines(malformed: bool = False) -> tuple[Any, Any]:
...
class TestDSPy:
    def test_real_copro_preserves_native_prefix_and_labels_training_history(
        self, tmp_path: Path
    ) -> None:
        ...
        training_events = [
            e for e in result.history if e["data"].get("evaluated_split") == "train"
        ]
    ...
    def test_preconfigured_optimizer_and_program_are_used_without_rebuilding(
        self, tmp_path: Path
    ) -> None:
        dspy = pytest.importorskip("dspy")
    ...
    def test_native_multistage_module_keeps_all_predictor_state(self, tmp_path: Path) -> None:
        dspy = pytest.importorskip("dspy")

        class TwoStages(dspy.Module):
            def __init__(self) -> None:
                super().__init__()
                self.first = dspy.Predict(
                    dspy.Signature("text -> answer", instructions="first seed")
                )
                self.second = dspy.Predict(
                    dspy.Signature("text -> answer", instructions="second seed")
                )
        ...
        result = optimize(
            ...
            model=model,
            backend=DSPy(
                optimizer="LabeledFewShot", optimizer_kwargs={"k": 1}, program=TwoStages()
            ),
...
class TestTextGrad:
    ...
    def test_textgrad_optimizer_factories_and_custom_loss_keep_backward_graph(
        ...
        model, critic = _engines()
        loss_inputs = []

        def loss(response: Any, example: Example, engine: Any) -> Any:
            loss_inputs.append((response.requires_grad, example.input))
            return tg.TextLoss("CUSTOM RUBRIC: return the affirmative label", engine=engine)(
                response
            )

        def optimizer(**kwargs: Any) -> Any:
    ...
    def test_textgrad_string_loss_and_malformed_update_are_recorded(self, tmp_path: Path) -> None:
        model, critic = _engines(malformed=True)
