...
from examples.career_coaching import calibrate_judge as stage1
...
class TestCoachingCalibration:
    def test_response_stage_uses_snapshot_and_saved_judge_settings(
    ...
    def test_response_stage_rejects_invalid_calibration_dataset(
        self, tmp_path: Path, fault: str
    ) -> None:
        config = _calibration_config()
        metadata = config["metadata"]
        if fault == "overlap":
            metadata["question_splits"]["response_test"] = metadata["question_splits"][
                "judge_train"
            ]
        elif fault == "missing":
    ...
    def test_calibration_report_is_uploaded_including_when_seed_is_retained(
        self, tmp_path: Path, keep_seed: bool
    ) -> None:
        class Backend:
    ...
    def test_response_report_and_verdicts_are_ready_for_upload(self, tmp_path: Path) -> None:
        ...
        optimize(
            ...
            test_data=[Example("test", incumbents["test"])],
            metric=metric,
            metadata={"judge": "best", "judge_run": "stage1"},
            output_dir=tmp_path / "run",
